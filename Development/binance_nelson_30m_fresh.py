...
# params
params = {}
params['datapoints_trailing'] = datapoints_trailing = 30
params['trailing_price_range_max_percent'] = trailing_price_range_max_percent = 0.20
params['trail_slope_percent_min'] = trail_slope_percent_min = 0.15
params['sell_skip'] = sell_skip = 15
params['sell_retract_by'] = sell_retract_by = 0.2
params['future_window'] = future_window = 30
params['trail_mov_avg_percent_min'] = trail_mov_avg_percent_min = 0.005

params['movement_multiplier'] = movement_multiplier = 1
params['movement_multiplier_ceiling'] = movement_multiplier_ceiling = 100
params['movement_percent'] = movement_percent = 0.00
pprint(params)
        ...
        if symbol['quoteAsset'] == 'BTC' and test_symbols:
            ...
            for index, candle in enumerate(data):
                ...
                if len(trailing_volumes) >= datapoints_trailing:
                    ...
                    d['current_movement'] = current_movement = float(candle[2])-float(candle[1])
                    ...
                    d['current_percent'] = current_percent = round(current_movement / float(candle[1]), 3)

                    # conditions
                    d['green_bar'] = green_bar = current_movement > 0
                    d['trail_mov_avg_percent_min_ok'] = trail_mov_avg_percent_min_ok = trail_mov_avg_percent > trail_mov_avg_percent_min
                    d['range_ok'] = range_ok = trail_price_range_percent < trailing_price_range_max_percent
                    d['mov_ceiling_ok'] = mov_ceiling_ok = current_movement < movement_multiplier_ceiling * trail_mov_avg
                    d['mov_ok'] = mov_ok = current_movement > movement_multiplier * trail_mov_avg
                    d['mov_price'] = mov_price = float(candle[1]) + movement_multiplier * trail_mov_avg
                    d['mov_percent_ok'] = mov_percent_ok = current_movement / float(candle[1]) > movement_percent
                    d['mov_percent_price'] = mov_percent_price = float(candle[1]) + float(candle[1]) * movement_percent
                    d['slope_ok'] = slope_ok = (trailing_opens[-1] - trailing_opens[0]) / trailing_opens[0] > trail_slope_percent_min
                    ...
                    # catch a timestamp
                    if False and symbol['symbol'] == 'ARNBTC':
                        if readable_time == '2018-01-04 18:30:00':
                            print(symbol['symbol'], readable_time)
                            # pprint(c)
                    ...
                    if slope_ok and range_ok and mov_ceiling_ok and mov_ok and mov_percent_ok and no_trade_overlap and green_bar and trail_mov_avg_percent_min_ok:
                        #if range_ok and trail_mov_avg_percent_min_ok and slope_ok and no_trade_overlap:
