...
# 777
def get_optimization_factors(optimizing_array, data_length_multiplier, end_time, symbol, symbols_trimmed_one, length, minutes, minutes_until_sale, compare_price_type):
            ...
            if compare_price_type == 0:
                ...
                try:
                    if optimizing == 1:
                        ...
                        for x in range(1,optimizing+1):
                            total_of_prices += float(data[index-x][4])*x
                            total_counts_of_prices += x
                        
                        # for x in range(1,optimizing):
                        #     total_of_prices += float(data[index-2*optimizing+x][4])*x
                        #     total_counts_of_prices += x
                except Exception as e:
                    print(e)
                    continue  
...
def run_optimizer_multi(lengths, compare_price_type):
    while True:
                ...
                for s in symbols_trimmed:
                    ...
                    for lb in results_by_look_back:
                        ...
                        if compare_price_type == 0:
                            ...
                            optimization_factors_path_2 = '/home/ec2-user/environment/botfarming/Development/optimization_factors/2_' + length + '_optimal_for_' + symbol['symbol'] + '_' + str(result['look_back']) + '.pklz'
