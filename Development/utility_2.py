...
def run_24hr_1min_drop_strategy(bot_index=-1, total_bots=-1):
    print('starting 24hr_1min_drop_strategy')
    print_computer_ip()
    min_volume_btc = 0
    ...
    print('trading on', len(symbol_list), 'symbols')
    for s in symbol_list:
        symbol = symbols_filtered[s]
        t = Thread(target=start_24hr_1min_drop_thread, args=[symbol])
        t.start()


def start_24hr_1min_drop_thread(symbol):
    while True:
        monitor_and_buy_for_24hr_1min_drop(symbol, interval='1m', file_number=2000)


def monitor_and_buy_for_24hr_1min_drop(symbol, interval, file_number):
    s = symbol['symbol']

    # once every 10 min ensure we are still running
    # if (symbol['symbol'] == 'ETHBTC') and (time.localtime().tm_min%10 == 0 and time.localtime().tm_sec == 1 or time.localtime().tm_sec == 2):
    #     print('10min CHECK ALIVE monitor_and_buy_for_24hr_1min_drop()', symbol['symbol'], ut.get_time())

    # once an hour print out all the order books to ensure they are all running still
    # if time.localtime().tm_min == 2:
    #     order_book = ut.pickle_read('./recent_order_books/'+symbol['symbol'] +'.pklz')
    #     try:
    #         print('ORDER BOOK STILL RUNNING', symbol['symbol'], order_book['bids'][0][0], ut.get_time())
    ...
    current_state_path = './program_state_' + interval + '/program_state_' + interval + '_' + str(file_number) + '_' + s + '.pklz'
    ...
    try:
        ...
        # TODO: get from disk after auto-optimizer saves
        ...
        # buy_trigger_drop_percent_factor = 0.7 # starter
        # sell_trigger_gain_percent_factor = 0.6 # starter
        # buy_trigger_drop_percent_factor = 0.6 # starter faster
        # sell_trigger_gain_percent_factor = 0.5 # starter faster
        buy_trigger_drop_percent_factor = 0.9 # starter faster   10 trades/hr
        sell_trigger_gain_percent_factor = 0.8 # starter faster


        # generated from previous 24 hours via cron
        symbol_24hr_drop_path = './binance_24hr_1min_drop/24hr_1min_drops_by_symbol/'+s+'.pklz'
        ...
        # continue only once we have a partial order book for the current min (accurate open price, API lag)
        while time.localtime().tm_sec < 3:
            time.sleep(.1)

        # get 2 recent candles
        ...
        # run for the remaing 57 + 1 seconds of the minute
        while time.localtime().tm_sec < 1 or time.localtime().tm_sec >= 3:

            order_book = ut.get_order_book_local(symbol['symbol'])
            time.sleep(.3)
            ...
            # start buy process a bit early (to be faster to the line)
            if drop_percent >= buy_trigger_drop_percent * 0.995:
                ...
                price_to_start_buy = price_to_buy * 1.005
                price_to_start_sell = price_to_sell * 0.995
                ...
                largest_bitcoin_order = 0.1 # part_of_bitcoin_to_use * 0.1
                amount_to_buy = part_of_bitcoin_to_use / price_to_buy
                # largest_buy_segment = largest_bitcoin_order / price_to_buy
                largest_buy_segment = part_of_bitcoin_to_use / price_to_buy # buy in one big order for now
                ...
                current_state['trailing_candles'] = candles
                ...
                # DEBUGGERS
                # print('amount_to_buy', ut.float_to_str(amount_to_buy), 'min_quantity', ut.float_to_str(current_state['min_quantity']) )
                # print('part_of_bitcoin_to_use', ut.float_to_str(part_of_bitcoin_to_use) )
                # break

                # buy as much as possible for 60 seconds
                time_to_give_up = int(time.time()) + 60
                price_to_buy_max = price_to_buy
                amount_to_stop_buying = amount_to_buy
                while True:
                    # check if order filled & update state
                    if current_state['orderId'] != False:
                        ...
                        if buy_order_info['status'] == 'FILLED':
                            ...
                            # check if total order filled
                            if current_state['executedQty'] >= amount_to_stop_buying:
                                break

                    # if 60 seconds have elapsed, start selling
                    if int(time.time()) >= time_to_give_up:
                        if current_state['orderId'] != False:
                            current_state = ut.cancel_buy_order(current_state)
                        break

                    # cancel buy order if price is out of range
                    first_in_line_price, first_bid, second_in_line_price, second_bid, second_price_to_check = ut.get_first_in_line_price_buying(current_state)
                    if float(first_in_line_price) > price_to_buy_max:
                        if current_state['orderId'] != False:
                            ...
                            if current_state['executedQty'] >= amount_to_stop_buying:
                                break
                    # otherwise keep trying to buy at front of order book
                    else:
                        # buy at price to buy, don't worry about being first in order book
                        if current_state['orderId'] == False:
                            current_state = ut.create_buy_order(current_state, price_to_buy_for_order)

                        # # make new buy order @ largest_bitcoin_order
                        # if current_state['orderId'] == False:
                        #     current_state = ut.create_buy_order(current_state, first_in_line_price)
                        # # else, update buy orders to smartest price possible
                        # else:
                        #     buy_order_info = current_state['client'].get_order(symbol=current_state['symbol'], orderId=current_state['orderId'])
                        #     # if we aren't first in line, cancel & get first inline
                        ...
                        #         current_state = ut.create_buy_order(current_state, first_in_line_price)
                        #     else:
                        #         # if we are first in line but our price is too high, lower it to near second in line price
                        #         if second_price_to_check > second_bid:
                        #             current_state = ut.cancel_buy_order(current_state)
                        #             if current_state['executedQty'] >= amount_to_stop_buying:
                        #                 break
                        #             current_state = ut.create_buy_order(current_state, second_in_line_price)
                    time.sleep(.03)
...
def sell_for_24hr_1min_drop(current_state):
    # print('------------------START SELLING', current_state['symbol'], '@', ut.float_to_str(current_state['price_to_sell'], 8), 'bought @', ut.float_to_str(current_state['price_to_buy'], 8), current_state['original_buy_time_readable'], ut.get_time())
    ...
    try:
        # try to sell until the end of future_candles while price is high enough, then go to market
        while True:
            order_book = ut.get_order_book_local(current_state['symbol'])
            current_price = float(order_book['bids'][0][0])
            # start selling early
            price_ok_to_sell = current_price >= current_state['price_to_sell'] * 0.995
            ...
            if print_current_price:
                print('---------------START SELLING', current_state['symbol'], 'current', ut.float_to_str(current_price, 8), 'selling', ut.float_to_str(current_state['price_to_sell'], 8), 'bought', ut.float_to_str(current_state['price_to_buy'], 8), current_state['original_buy_time_readable'], ut.get_time())
                print('time_to_give_up', time_to_give_up, ut.get_readable_time(time_to_give_up))
                print_current_price = False

            # if sale order was filled & update state
            if current_state['orderId'] != False:
            ...
            # out of time, sell at market      X best order book price until it is sold
            if int(time.time()) >= time_to_give_up:
                # if sale order exists cancel it
                if current_state['orderId'] != False:
                    current_state = ut.cancel_sale_order(current_state)
                    if current_state['executedQty'] < current_state['min_quantity']:
                        break
                price_ok_to_sell = True
                # current_state['price_to_sell'] = 0
                current_state = ut.create_sale_order(current_state, current_state['executedQty'], market=True)
                ut.pickle_write(current_state_path, current_state, '******could not reduce price_to_sell to zero******')

            # otherwise, keep trying to sell for profit!
            if price_ok_to_sell:
                first_in_line_price, first_ask, second_in_line_price, second_ask, second_price_to_check = ut.get_first_in_line_price(current_state)

                # sell at price to sell, don't worry about order book
                if current_state['orderId'] == False:
                    if current_state['price_to_sell'] == 0:
                        current_state = ut.create_sale_order(current_state, current_state['executedQty'], market=True)
                    else:
                        current_state = ut.create_sale_order(current_state, price_to_sell_for_order)

                # # no existing order, start or continue selling
                # if current_state['orderId'] == False:
                #     current_state = ut.create_sale_order(current_state, first_in_line_price)
                # else:
                # # update price on existing order (intelligent order book positioning)
                #     sale_order_info = current_state['client'].get_order(symbol=current_state['symbol'],orderId=current_state['orderId'])
                #     # if we aren't first in line cancel & start again
                ...
                #         current_state = ut.create_sale_order(current_state, first_in_line_price)
                #     else:
                #     # if we are first in line, move price up to just below second in line price
                #         if second_price_to_check < second_ask:
                #             current_state = ut.cancel_sale_order(current_state)
                #             if current_state['executedQty'] < current_state['min_quantity']:
                #                 break
                #             current_state = ut.create_sale_order(current_state, second_in_line_price)
            time.sleep(.03)

        # sale is complete (it always completes), record the trade
        ut.calculate_profit_and_free_coin(current_state, strategy='24hr_1min_drop')
        return True
    except Exception as e:
        print(e)
...
def save_data(save_params, datapoints_trailing, min_volume, minutes):
    for settings in save_params:
        ...
        end_time = arrow.get(settings[2]).replace(tzinfo='America/Denver')
            # file names ==> './binance_training_data/'+ day_folder + '/'+ symbol +'_data_'+str(minutes)+'m.pklz'
        ...
        if start_end_style:
            ...
            for symbol in coins:
                ...
                if symbol == 'ETHBTC':
                    ...
                    pprint(data[-1])
                    print('--------------------ETHBTC')
                ut.pickle_write('./binance_training_data/'+ day_folder + '/'+ symbol +'_data_'+str(minutes)+'m.pklz', data)
