    ...
    try:
        ...
        while time.localtime().tm_sec >= 2 and have_open_price:
            ...
            if drop_percent >= buy_trigger_drop_percent:
                ...
                # print('-------------------BUY', s, '@', ut.float_to_str(price_to_buy, 8), 'sell @', ut.float_to_str(price_to_sell, 8), ut.get_time(), 'open', ut.float_to_str(open_price, 8), 'cur', ut.float_to_str(current_price, 8), 'drop', ut.float_to_str(drop, 8), ut.get_time())
                ...
                # STOP TIME limit time window for testing
                t = datetime.datetime(2018, 2, 15, 19, 50)
                s = int((t - datetime.datetime(1970, 1, 1)).total_seconds()) + 7 * 60 * 60
                epoch_now = int(time.time())
                diff = epoch_now - s
                if diff > 0:
                    print('-----------------TIME WINDOW EXPIRED no more buys', ut.get_time())
                    return False
                ...
                # largest_bitcoin_order = part_of_bitcoin_to_use
                ...
                ## Let's handle min_notional correctly here !!!
                # min_quantity = max(float(symbol['filters'][1]['minQty']), float(symbol['filters'][2]['minNotional']))
                # quantity_decimals = ut.get_min_decimals(min_quantity)
                # print('quantity_decimals == quantity_decimals_2', quantity_decimals == quantity_decimals_2, quantity_decimals, quantity_decimals_2)
...
def trade_on_drops(symbol, data, future_candles_length, buy_trigger_drop_percent, sell_trigger_gain_percent, btc_tradeable_volume_factor, btc_usd_price=9400):
    s = symbol['symbol']
    one_min_avg_volume = float(symbol['24hourVolume']) / (24*60)
    btc_tradeable_volume =  btc_tradeable_volume_factor * one_min_avg_volume
    trades = []
